...
struct CubebSink::Impl {
    ...
    static long DataCallback(cubeb_stream* stream, void* user_data, const void* input_buffer, void* output_buffer, long num_frames);
...
CubebSink::CubebSink() : impl(std::make_unique<Impl>()) {
    ...
    if (cubeb_stream_init(impl->ctx, &impl->stream, "Citra Audio Output", nullptr, nullptr, output_device,
                        &params, std::max(1024u, minimum_latency), &Impl::DataCallback,
...
long CubebSink::Impl::DataCallback(cubeb_stream* stream, void* user_data, const void* input_buffer, void* output_buffer, long num_frames) {
    ...
    if(frames_to_write < num_frames) {
        ...
        memset(buffer + frames_to_write * sizeof(s16) * 2, 0, (num_frames - frames_to_write) * sizeof(s16) * 2);
    }
